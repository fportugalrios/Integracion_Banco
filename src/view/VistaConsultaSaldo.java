...
public class VistaConsultaSaldo extends JFrame{
	...
	public VistaConsultaSaldo() {
		...
		JLabel lblSaldosDeCuenta = new JLabel("Saldos de cuenta");
		lblSaldosDeCuenta.setBounds(24, 67, 130, 16);
		getContentPane().add(lblSaldosDeCuenta);
		
		JTextArea textAreaSaldos = new JTextArea();
		textAreaSaldos.setBounds(24, 95, 406, 157);
		getContentPane().add(textAreaSaldos);
		textAreaSaldos.setEnabled(false);
		
		JLabel labelResultado = new JLabel("");
		labelResultado.setBounds(24, 280, 161, 18);
				...
				try{
					...
					if (c!=null) {
						...
						for (int i=0; i<cuentasAux.size(); i++) {
							...
							String cadenaSaldo = "Cuenta nro: " + cta.getNumero() + " - " + "Saldo: " + cta.getSaldo();
							textAreaSaldos.setText(cadenaSaldo);
							textAreaSaldos.setText("\n");
						}
					} else {
